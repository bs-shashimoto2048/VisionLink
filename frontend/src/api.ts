...
export async function lookupInternalData(payload: {
  qrText?: string;
  orderNo?: string;
  serialNo?: string;
  terminalName?: string;
...
export async function fetchCheckSerials() {
  return requestJson<{ serials: string[] }>("/api/check-data/serials");
}

export async function fetchCheckBoards(serial: string) {
  return requestJson<{ boards: string[] }>(`/api/check-data/boards?serial=${encodeURIComponent(serial)}`);
}

export async function fetchCheckTerminals(serial: string, board: string) {
  return requestJson<{ terminals: string[] }>(
    `/api/check-data/terminals?serial=${encodeURIComponent(serial)}&board=${encodeURIComponent(board)}`
  );
}
...
export async function analyzeFrame(payload: {
  ...
  rotateLabelOcr?: boolean;
}) {
  const formData = new FormData();
  formData.append("session_id", payload.sessionId);
  formData.append("operator_id", payload.operatorId);
  ...
  console.debug("[VisionLink] frame-analyze response", {
    frame_index: response.frame_index,
    detections_count: response.detections.length,
    ocr_results_count: response.ocr_results.length,
...
export async function updateRow(payload: {
  ...
  return requestJson<InspectionSessionResponse>(
    `/api/inspection/session/${payload.sessionId}/rows/${payload.lineNo}/manual-edit`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
export async function fetchSession(sessionId: string) {
  return requestJson<InspectionSessionResponse>(`/api/inspection/session/${sessionId}`);
}

export async function pauseInspection(payload: { sessionId: string; employeeId: string }) {
  return requestJson<InspectionSessionResponse>(`/api/inspection/session/${payload.sessionId}/pause`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ employee_id: payload.employeeId }),
  });
}

export async function resumeInspection(payload: { sessionId: string; employeeId: string }) {
  return requestJson<InspectionSessionResponse>(`/api/inspection/session/${payload.sessionId}/resume`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ employee_id: payload.employeeId }),
  });
...
export async function abortInspection(payload: { sessionId: string; employeeId: string }) {
  return requestJson<InspectionSessionResponse>(`/api/inspection/session/${payload.sessionId}/abort`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ employee_id: payload.employeeId }),
  });
